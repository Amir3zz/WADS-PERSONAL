import { GET, PATCH, DELETE } from "@/app/api/cards/[cardId]/route";
...
const mockContext = {
  params: Promise.resolve({ cardId: "test-card-id" }),
};

describe("Card API Security Tests", () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  test("PATCH should return 400 for invalid position", async () => {
    (auth.getSession as jest.Mock).mockResolvedValue({ id: "user1" });
    (prisma.card.findFirst as jest.Mock).mockResolvedValue({ id: "test" });
  ...
  test("DELETE should return 404 if card not found", async () => {
    (auth.getSession as jest.Mock).mockResolvedValue({ id: "user1" });
